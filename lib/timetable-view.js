...
(function (TTV) {
    ...
    function buildReplacer(replacerRules) {
    ...
    function buildEventsView(viewJson) {
        ...
        const shaders = (() => {
            ...
            for (const shaderKey in viewJson["shaders"]) {
                const re = buildRegExp(viewJson["shaders"][shaderKey]);
        ...
        function evalToken(state, token) {
            ...
            if (typeof token == "string") {
                ...
                switch ( /*fn_name*/token[0]) {
                    ...
                    case "shader": {
                        ...
                        const shaderKey = token[2].toString();
                        const matchNum = parseInt(token[3].toString());
                        const groupNum = parseInt(token[4].toString());
                    ...
                    // UNSAFE
                    case "<span>": {
                        const content = evalToken(state, token[1]);
                        const classList = encodeHTML((_f = token[2]) !== null && _f !== void 0 ? _f : "");
