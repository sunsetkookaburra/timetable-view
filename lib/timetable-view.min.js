...
function createHTMLElement(selector, children = []) {
    var _a;
    ...
    while ((match = re.exec(selector)) != null) {
        switch (match[0][0]) {
            ...
            case ".":
                ...
                break;
            case "[":
                attrs.push([match[1], (_a = match[2]) !== null && _a !== void 0 ? _a : ""]);
                break;
            default:
                tag = match[0];
                break;
    ...
    class Runtime {
        ...
        execute(state, program) {
            ...
            for (const chunk of program) {
                const result = this.evalChunk(state, chunk);
                if (result.ok)
                    out.push(result.value);
            }
            return out;
        }
        evalChunk(state, chunk) {
            let value;
            let ok;
            // chunk == SourceCodeFn
            if (chunk instanceof Array) {
                // toString as a soft fallback
                const fnId = chunk[0].toString();
...
(function (TTV) {
    ...
    function buildEventsView(viewJson) {
        ...
        const viewsRT = new ArrayScript.Runtime(primitive => { var _a; return document.createTextNode((_a = primitive === null || primitive === void 0 ? void 0 : primitive.toString()) !== null && _a !== void 0 ? _a : "null"); });
